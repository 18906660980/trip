...
const missing = [
  { name: "renmin", q: "Chengdu People Park teahouse" },
  { name: "chuanchuan", q: "Chuan chuan xiang Sichuan" },
  { name: "chaoshou", q: "Chengdu wonton red oil" },
  { name: "tuto", q: "Rabbit head Sichuan snack spicy" },
  { name: "tianshui", q: "Sweet water noodles" },
  { name: "bingfen", q: "Ice jelly Chinese dessert" },
];

(async () => {
  for (const item of missing) {
    const urls = await searchFiles(item.q);
    console.log(item.name + ": " + JSON.stringify(urls));
