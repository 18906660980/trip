...
async function searchFiles(searchTerm) {
  try {
    const r = await proxyGet("https://commons.wikimedia.org/w/api.php?action=query&list=search&srsearch=" + encodeURIComponent(searchTerm) + "&srnamespace=6&srlimit=5&format=json");
    const json = JSON.parse(r);
    if (!json.query || !json.query.search) return [];
    const titles = json.query.search.map(s => s.title).join("|");
    const r2 = await proxyGet("https://commons.wikimedia.org/w/api.php?action=query&titles=" + encodeURIComponent(titles) + "&prop=imageinfo&iiprop=url|mime&iiurlwidth=800&format=json");
    const json2 = JSON.parse(r2);
    const pages = json2.query ? Object.values(json2.query.pages) : [];
    return pages.filter(p => p.imageinfo && p.imageinfo[0].mime && p.imageinfo[0].mime.startsWith("image/")).map(p => p.imageinfo[0].thumburl || p.imageinfo[0].url);
  } catch(e) { return []; }
}

const missing = [
  { name: "chuanchuan", q: "skewers hot pot Chinese" },
  { name: "chaoshou", q: "wonton chili oil Chinese food" },
  { name: "tuto", q: "spicy rabbit head" },
  { name: "bingfen", q: "grass jelly dessert" },
  { name: "hotpot2", q: "Sichuan spicy hot pot mala" },
];

(async () => {
  for (const item of missing) {
    const urls = await searchFiles(item.q);
    console.log(item.name + ": " + JSON.stringify(urls.slice(0,3)));
